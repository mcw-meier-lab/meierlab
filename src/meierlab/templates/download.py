...
class XNATDownloadTemplate(BaseDownloadTemplate):
    ...
    def _validate_config(self) -> None:
        ...
        if not self.config.get("dry_run", False):
            if not self.config.get("username") or not self.config.get("password"):
                raise ValueError(
                    "Username and password are required for XNAT authentication"
                )
    ...
    def _add_arguments(self, parser):
        ...
        parser.add_argument(
            "-p", "--project", help="XNAT project name", default=self.config["project"]
        )
        parser.add_argument("-un", "--username", help="XNAT username", required=True)
        parser.add_argument("-up", "--password", help="XNAT password", required=True)
        parser.add_argument(
            "-wd",
            "--working-directory",
        ...
        parser.add_argument(
            "-d", "--dcm2nii", help="Convert DICOM to NIfTI", action="store_true"
        )
        parser.add_argument(
            "-b", "--bids", help="Convert to BIDS format", action="store_true"
        )
        parser.add_argument(
            "-sc",
            "--scan",
                ...
                with open(self.config["list_file"]) as f:
                    ...
                    for line in lines:
                        if (
                            ...
                            should_include = True
                            break
            elif self.config["subject"] != "all" and self.config["experiment"] != "all":
                if (
                    self.config["subject"] == subj_label
                    and self.config["experiment"] == exp_label
                ):
                    should_include = True
            else:
                should_include = True
    ...
    def _setup(self, download_dict: dict[str, Any]) -> None:
        ...
        for bids_label, experiment in download_dict.items():
            ...
            tmp_dir = os.path.join(
                self.config["working_directory"], "sourcedata", bids_label
            )
            ...
            try:
                with ZipFile(
                    f"{self.config['working_directory']}/{bids_label}.zip", "r"
                ) as ff:
