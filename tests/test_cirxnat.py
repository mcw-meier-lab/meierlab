...
@pytest.mark.skipif("CIR2" not in os.environ, reason="local testing only")
def test_get_json_data(example_server, example_subject_id, example_experiment_id):
    json_data = example_server.get_json_data(
        example_subject_id, example_experiment_id, ["T1W"]
    )
